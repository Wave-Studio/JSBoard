	...
	return (
		<>
			<div className={"has-tooltip relative nightwind-prevent " + props.className} onClick={() => nightwind.enable(darkModeLoc)}>
				...
				<span className={"tooltip nightwind-prevent rounded font-medium shadow-lg p-1 bg-opacity-30 backdrop-filter backdrop-blur-3xl backdrop-saturate-150  mt-14 whitespace-nowrap -left-14 " + (props.darkTextOnLight && !darkModeLoc ? "text-gray-900 bg-gray-300" : "text-gray-100 bg-gray-200")}>Switch to {darkModeLoc ? "light" : "dark"} mode</span>
			</div>
		</>
	)
}